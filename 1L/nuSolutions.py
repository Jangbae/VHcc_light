...
def intersections_ellipse_line(ellipse, line, zero=1e-12):
    ...
    sols = sorted([(v.real / v[2].real, np.dot(line,v.real)**2 + np.dot(v.real,ellipse).dot(v.real)**2) for v in V.T], key=lambda (s, k): k)[:2]
...
#         print "self.solutions[0] : ", self.solutions[0]
        if len(self.solutions)<1:
            return False
        else:
